...
const FarmPage: NextPage = () => {
  ...
  useEffect(() => console.log("chain", chain), [chain]);
      ...
      {!isLoading && idQuery ? (
        <div className="px-6 sm:px-[72px] text-[#475467] z-10">
          ...
          <div className="flex flex-col bg-white rounded-lg p-6 sm:pb-24 sm:pt-[69px] md:pb-24 sm:px-11 lg:pl-[51px] lg:pr-[76px]">
            ...
            {!!chain && chain.network !== farm?.chain && (
              <div className="flex flex-col gap-y-1">
                <span>On Wrong chain</span>
                <Button
                  size="large"
                  style="mb-6 max-w-fit bg-red-100 text-red-500 hover:bg-red-200 transition duration-200 shadow-md"
                  onButtonClick={() => {
                    switchNetwork?.(getChainId(farm?.chain));
                  }}
                >
                  Switch to {formatFirstLetter(farm?.chain)}
                </Button>
              </div>
            )}
              ...
              <div className="w-full sm:w-1/2 flex flex-col gap-y-3">
                ...
                <p className="absolute">8</p>
                  ...
                  <div className="py-4 px-6 text-sm leading-5">
                    The score is a relative indicator of the reliability of the
                    farm compared to the other opportunities listed on YieldBay.
                    The score is a calculated based on the current TVL, APR, and
                    the rewards being dished out in the farm.
                  </div>
                </div>
              </div>
                      ...
                      {farm?.rewards.map((reward, index) => (
                        ...
                        >
                          ...
                          <td className="py-[14px] pl-6 underline underline-offset-4 decoration-dotted decoration-3 decoration-[#475467] cursor-default">
                            <Tooltip
                              label={
                                <div>{"$" + reward.valueUSD.toFixed(1)}</div>
                              }
                            >
                              <>
                                {parseFloat(
